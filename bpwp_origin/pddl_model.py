...
from .util import Condition, ConditionType, Effect, EP_formula, EPFType, Action, UpdateType
from .util import extract_v_from_s, multiple_parameter_replace, multiple_parameter_replace_with_ep, updateEffect, \
    global_state_evaluation, special_value
...
class Problem:
    def __init__(self, domain_name, problem_name, entities, types: typing.Dict[str, Type],
                 function_schemas: typing.Dict[str, FunctionSchema],
                 action_schemas: typing.Dict[str, ActionSchema], rules, functions: typing.Dict[str, Function],
                 initial_state, goals, external=None, handlers=None):
        ...
        self.epistemic_model = epistemic_model.EpistemicModel(handlers, self.entities, self.functions,
                                                              self.function_schemas, self.external)
    ...
    def generate_successor(self, state: typing.Dict[str, any], action: Action, previous_path):
        ...
        for effect_name, item in action.effects.items():
            ...
            if effect.update_type == UpdateType.CONSTENT:
                ...
                new_value = updateEffect(self.logger, effect.effect_type, value1, value2,
                                         self.function_schemas[function_schema_name])
                if new_value == None:
                    return None
        ...
        for action_schema_name, action_schema in self.action_schemas.items():
            ...
            for parameter_replacement in new_parameter_replacement_list:
                ...
                for precondition_name, precondition_item in action_schema.preconditions.items():
                    ...
                    new_precondition_name = multiple_parameter_replace_with_ep(precondition_name, parameter_replacement,
                                                                               VARIABLE_FILLER)
                    new_precondition = Condition()
                    new_precondition.condition_operator = precondition_item.condition_operator
                    if not precondition_item.target_variable is None:
                        new_precondition.target_variable = multiple_parameter_replace(precondition_item.target_variable,
                                                                                      parameter_replacement,
                                                                                      VARIABLE_FILLER)
                    elif not precondition_item.target_value is None:
                        new_precondition.target_value = precondition_item.target_value
                    else:
                        raise ValueError("One of the condition target variable or value should not be None",
                                         precondition_name)

                    if precondition_item.condition_type == ConditionType.ONTIC:
                        new_precondition.condition_type = ConditionType.ONTIC
                        new_precondition.condition_variable = multiple_parameter_replace(
                            precondition_item.condition_variable, parameter_replacement, VARIABLE_FILLER)
                        ...
                        new_ep_formula.ep_query = multiple_parameter_replace(old_ep_formula.ep_query,
                                                                             parameter_replacement, "")

                        if new_ep_formula.epf_type == EPFType.EP:
                            new_ep_formula.ep_formula_str = multiple_parameter_replace_with_ep(
                                old_ep_formula.ep_formula_str, parameter_replacement, "")
                            ...
                            new_varphi.condition_variable = multiple_parameter_replace(old_varphi.condition_variable,
                                                                                       parameter_replacement,
                                                                                       VARIABLE_FILLER)
                            if old_varphi.target_variable is None:
                                new_varphi.target_value = old_varphi.target_value
                            elif old_varphi.target_value is None:
                                new_varphi.target_variable = multiple_parameter_replace(old_varphi.target_variable,
                                                                                        parameter_replacement,
                                                                                        VARIABLE_FILLER)
                            else:
                                raise ValueError("One of the condition target variable or value should not be None",
                            ...
                            new_ep_formula.ep_variable = multiple_parameter_replace(old_ep_formula.ep_variable,
                                                                                    parameter_replacement,
                                                                                    VARIABLE_FILLER)
                ...
                for effect_name, item in action_schema.effects.items():
                    ...
                    new_effect = Effect()
                    new_effect_name = multiple_parameter_replace_with_ep(effect_name, parameter_replacement,
                                                                         VARIABLE_FILLER)
                    new_effect.effect_type = effect_item.effect_type
                    new_effect.target_variable_name = multiple_parameter_replace(effect_item.target_variable_name,
                                                                                 parameter_replacement, VARIABLE_FILLER)
                    ...
                    if new_effect.update_type == UpdateType.CONSTENT:
                        ...
                        new_effect.update = multiple_parameter_replace(effect_item.update, parameter_replacement,
                                                                       VARIABLE_FILLER)
                    elif new_effect.update_type == UpdateType.EPSITEMIC:
                        new_ep_formula = EP_formula()
                        old_ep_formula = effect_item.update
                        new_ep_formula.epf_type = old_ep_formula.epf_type
                        new_ep_formula.ep_query = multiple_parameter_replace_with_ep(old_ep_formula.ep_query,
                                                                                     parameter_replacement, "")
                        if new_ep_formula.epf_type == EPFType.EP:
    ...
    def checking_action_by_ontic_preconditions(self, preconditions: typing.Dict[str, Condition], parameter_replacement,
                                               current_state):
        for precondition_name, precondition_item in preconditions.items():
            # self.logger.debug(precondition_name)
            operator = precondition_item.condition_operator
            variable_name = multiple_parameter_replace(precondition_item.condition_variable, parameter_replacement,
                                                       VARIABLE_FILLER)

            if not "?" in variable_name:
                value1 = extract_v_from_s(variable_name, current_state)
                if not precondition_item.target_variable is None:
                    target_variable_name = multiple_parameter_replace(precondition_item.target_variable,
                                                                      parameter_replacement, VARIABLE_FILLER)
                ...
                if not global_state_evaluation(self.logger, operator, value1, value2):
    ...
    def __str__(self):
        return (f"Domain name: {self.domain}\n"
                f"Problem name: {self.problem}\n\n"
                f"Entities: {self.entities}\n\n"
                f"Types: {self.types}\n\n"
                f"Function Schemas: {self.function_schemas}\n\n"
                f"Action Schemas: {self.action_schemas}\n\n"
                f"Rules: {self.rules}\n\n"
