...
class PDDLParser:
    ...
    def run(self, domain_path, problem_path):
        ...
        domain_name, problem_name, entities, types, function_schemas, action_schemas, rules, functions, initial_state, goals = self.problemParser(
            problem_str, domain_name, types, function_schemas, action_schemas)
        return domain_name, problem_name, entities, types, function_schemas, action_schemas, rules, functions, initial_state, goals

    def problemParser(self, problem_str, domain_name, types: typing.Dict[str, Type],
        ...
        problem_name, problem_str = self.keyWordParser("problem_name", PROBLEM_NAME_REG_PREFIX, PROBLEM_NAME_REG,
                                                       PROBLEM_NAME_REG_SURFIX, problem_str)
        ...
        agents_str, problem_str = self.keyWordParser("agents", AGENT_REG_PREFIX, AGENT_REG, AGENT_REG_SURFIX,
                                                     problem_str)
        ...
        # extract objects
        objects_str, problem_str = self.keyWordParser("objects", OBJECT_REG_PREFIX, OBJECT_REG, OBJECT_REG_SURFIX,
                                                      problem_str)
        ...
        ranges_str, len_holder = self.keyWordParser("ranges", RANGE_REG_PREFIX, RANGE_REG, RANGE_REG_SURFIX,
                                                    problem_str)
        problem_str = problem_str[:len(len_holder)]
        ...
        for function_schema in function_schemas.values():
            if not function_schema.content_type_list:
        ...
        for action_schema in action_schemas.values():
            ...
            for pre_str, precondition in action_schema.preconditions.items():
                if not precondition.target_value is None:
                    if precondition.condition_type == ConditionType.ONTIC:
                        function_schema_name = self.precondition_variable2function_schema_name(
                            precondition.condition_variable)
        ...
        for single_init_str in single_init_str_list:
            if not single_init_str.startswith("(assign"):
                raise ValueError("Error when parsing initial state [%s] (it should start with '(assign')",
                                 single_init_str)
            else:
        ...
        for goal_proposition_str in goal_proposition_str_list:
            ...
            if not operator_str in condition_operator_dict.keys():
                raise ValueError("Error when parsing goal [%s] (it should start with a valid operator)",
                                 goal_proposition_str)
            ...
            if "@jp" in goal_proposition_str:
                ...
                if value_str == "ep.true":
                    ...
                    raise ValueError("Ternary value type %s does not exist in %s when generate goal: %s", value_str,
                                     ternary_name_list, goal_proposition_str)
    ...
    def domainParser(self, domain_str):
        ...
        # extract domain name
        domain_name, domain_str = self.keyWordParser("domain_name", DOMAIN_NAME_REG_PREFIX, DOMAIN_NAME_REG,
                                                     DOMAIN_NAME_REG_SURFIX, domain_str)

        # extract typing
        typing_str, domain_str = self.keyWordParser("types", TYPING_REG_PREFIX, TYPING_REG, TYPING_REG_SURFIX,
                                                    domain_str)
        ...
        function_schemas: typing.Dict[str, FunctionSchema] = {}
        all_function_schema_str, domain_str = self.keyWordParser("function_schemas", FUNC_REG_PREFIX, FUNC_REG,
                                                                 FUNC_REG_SURFIX, domain_str)
        ...
        for action_str in action_str_list:
            ...
            action_content_str, _ = self.keyWordParser("action", ACTION_REG_PREFIX, ACTION_REG, ACTION_REG_SURFIX,
                                                       action_str)
            ...
            self.logger.debug(action_content_str)
            parameter_str, action_content_str = self.keyWordParser("parameters", PARAMETERS_REG_PREFIX, PARAMETERS_REG,
                                                                   PARAMETERS_REG_SURFIX, action_content_str)
            ...
            if not len(match_variables) == len(match_types):
                raise ValueError('missing variable or types in parameter [%s] for action [%s]', parameter_str,
                                 action_name)
            ...
            # extract effects
            effects_str, len_holder = self.keyWordParser("effect", EFFECT_REG_PREFIX, EFFECT_REG, EFFECT_REG_SURFIX,
                                                         action_content_str)
            ...
            preconditions_str, remaining_str = self.keyWordParser("precondition", PRECONDITION_REG_PREFIX,
                                                                  PRECONDITION_REG, PRECONDITION_REG_SURFIX,
                                                                  action_content_str)
            ...
            for precondition_str in precondition_str_list:
                ...
                if not operator_str in condition_operator_dict.keys():
                    raise ValueError("Error when parsing precondition [%s] (it should start with a valid operator)",
                                     precondition_str)
                new_condition = Condition()
                ...
                if "@jp" in precondition_str:
                    ...
                    if value_str == "ep.true":
                        ...
                        ternary_name_list = [n.name for n in Ternary]
                        raise ValueError("Ternary value type %s does not exist in %s when generate precondition: %s",
                                         value_str, ternary_name_list, precondition_str)
                    new_condition.condition_formula = new_ep_formula
                else:
                    new_condition.condition_type = ConditionType.ONTIC
    ...
    def parsingEffect(self, effect_str, action_name):
        ...
        if not effect_type_str in EFFECT_TYPE_DICT.keys():
            raise ValueError("Error effect [%s] for action [%s] is not a valid effect from: [%s]", effect_str,
                             action_name, EFFECT_TYPE_DICT.keys())
        ...
        if JP_PREFIX in effect_str:
            ...
            if not len(effect_content_list) == 2:
                raise ValueError("Error when parsing effect [%s] for action [%s]: an effect should have 2 components",
                                 effect_content_list, action_name)
            ...
            if not len(jp_content_list) == 2:
                raise ValueError("Error when parsing effect [%s] for action [%s]: a jp effect should have 2 components",
                                 jp_content_list, action_name)
            ...
            if not len(effect_content_list) == 3:
                raise ValueError(
                    "Error when parsing effect [%s] for action [%s]: a normal effect should have 3 components",
                    effect_str, action_name)
