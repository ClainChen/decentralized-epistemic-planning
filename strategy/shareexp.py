...
class ShareEXPGoalBFS(AbstractPolicyStrategy):
    """
        ...
        for virtual_model in all_virtual_model:
            ...
            for key, value in this_sample.items():
                if key in samples:
                    ...
                    samples[key] = value
        util.LOGGER.info(
            f"Models: {len(all_virtual_model)}, Expands: {expands}, {(((time.perf_counter() - start) / expands) * 1e3):.3f}ms/expand")
    ...
    def single_bfs(self, virtual_model: Model, agent_name: str):
        ...
        while heap:
            ...
            jp_world_for_agent_name = [f.id for f in util.get_epistemic_world(cur_model, [agent_name])]
            ...
            successors = {key: [succ for succ in value if util.is_valid_action(cur_model, succ)]
            ...
            for key, value in successors.items():
                if len(value) == 0:
                    successors[key] = cur_model.get_agent_successors(key)

            for name, succs in successors.items():
                for succ in succs:
                    if 'stay' in succ.name:
                        continue
