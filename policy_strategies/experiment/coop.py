...
from epistemic_handler.epistemic_class import Model, Action, Function, Condition
import heapq
import util
import logging
import random
...
class CoopBFS(AbstractPolicyStrategy):
    """
    1. Agents always assume that all other agents have the same goals as their own.
    """

    def get_policy(self, model: Model, agent_name: str) -> Action:
        model_copy = copy.deepcopy(model)
        cur_agt = model_copy.get_agent_by_name(agent_name)
        goals = []
        for goal in cur_agt.own_goals:
            g = copy.deepcopy(goal)
            g.belief_sequence[0] = agent_name
            g.belief_sequence = util.remove_continue_duplicates(g.belief_sequence)
            goals.append(g)
        goal_set = {agt.name: goals for agt in model.agents}
        cur_agt.all_possible_goals = [goal_set]
    ...
    def single_bfs(self, virtual_model: Model, agent_name: str):
        ...
        while heap:
            ...
            successors = {ca: node.model.get_agent_successors(ca) for ca in current_agent}
