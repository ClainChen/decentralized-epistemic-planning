    ...
    for task in tasks:
        ...
        for problem in problems:
            ...
            for test in tests:
                ...
                if strategy == 'share':
                    command = f"python entrance.py -d {domain}/domain.pddl -p {domain}/{problem['name']} -ob {ob} --rules {rules} --strategy {strategy}.py -tests {num_tests} > out/{domain}-{problem['name']}.out"
                elif strategy == 'stay':
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    ...
    for future in concurrent.futures.as_completed(future_to_command):
        ...
        try:
            ...
            # filename = f'{parse['strategy'][:-3]}.txt'
