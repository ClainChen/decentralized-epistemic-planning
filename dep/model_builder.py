    ...
    try:
        ...
        for agent_name in problem.agents:
            if agent_name in obs_func_mapper:
                obs_func_mapper[agent_name] = util.OBS_FUNC_FOLDER_PATH + obs_func_mapper[agent_name]
            else:
                obs_func_mapper[agent_name] = util.OBS_FUNC_FOLDER_PATH + args.observation_function
        ...
        for parsing_action in domain.actions:
            ...
            for parsing_condition in parsing_action.pre_conditions:
                ...
                if isinstance(parsing_condition, ParsingEpistemicCondition):
                    ...
                    new_condition_schema.ep_operator = EPISTEMIC_OPERATOR_MAPS[
                        parsing_condition.epistemic_logic_operator]
                ...
                new_condition_schema.condition_function_schema = model.get_function_schema_by_name(
                    parsing_condition.state.variable.name)
                ...
                if parsing_condition.state.value is not None:
                    ...
                    new_condition_schema.target_function_schema = model.get_function_schema_by_name(
                        parsing_condition.state.target_variable.name)
                    util.swap_param_orders(new_condition_schema.target_function_schema,
                                           parsing_condition.state.target_variable)
            ...
            for effect in parsing_action.effects:
                ...
                if effect.value is not None:
                    ...
                    new_effect_schema.target_function_schema = model.get_function_schema_by_name(
                        effect.target_variable.name)
        ...
        for agent in problem.agents:
            ...
            for parsing_goal in parsing_goals:
                ...
                new_goal.condition_function_parameters = dict(
                    zip(condition_function_schema.require_parameters.keys(), parsing_goal.state.variable.parameters))
                if not parsing_goal.state.value is None:
                    ...
                    new_goal.target_function_parameters = dict(zip(target_function_schema.require_parameters.keys(),
                                                                   parsing_goal.state.target_variable.parameters))
