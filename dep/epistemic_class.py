...
class Condition:
    ...
    def init_with_schema_and_params(cls, condition_schema: ConditionSchema, parameters: dict[str, str]):
        ...
        condition.condition_function_parameters = {key: parameters[key] for key in
                                                   condition_schema.condition_function_schema.require_parameters.keys()}
        if not condition_schema.value is None:
            ...
            condition.target_function_parameters = {key: parameters[key] for key in
                                                    condition_schema.target_function_schema.require_parameters.keys()}
        return condition
    ...
    def __eq__(self, other):
        ...
        return (self.ep_operator == other.ep_operator and
                ...
                frozenset(
                    self.condition_function_parameters.items() if self.condition_function_parameters else []) == frozenset(
                    other.condition_function_parameters.items() if other.condition_function_parameters else []) and
                self.target_function_name == other.target_function_name and
                frozenset(
                    self.target_function_parameters.items() if self.target_function_parameters else []) == frozenset(
                    other.target_function_parameters.items() if other.target_function_parameters else []) and
...
class Effect:
    ...
    def init_with_schema_and_params(cls, effect_schema: EffectSchema, parameters: dict[str, Entity]):
        ...
        effect.effect_function_parameters = {key: parameters[key] for key in
                                             effect_schema.effect_function_schema.require_parameters.keys()}
        if effect_schema.value is not None:
            ...
            effect.target_function_parameters = {key: parameters[key] for key in
                                                 effect_schema.target_function_schema.require_parameters.keys()}
            effect.target_function_belief_sequence = [parameters[key] for key in
                                                      effect_schema.target_function_belief_sequence]
        return effect
...
class Model:
    ...
    def get_function_schema_by_name(self, name: str) -> FunctionSchema:
        return copy.deepcopy(
            next((function_schema for function_schema in self.function_schemas if function_schema.name == name), None))
    ...
    def update_functions(self, effect: Effect):
        function = util.get_function_with_name_and_params(self.ontic_functions, effect.effect_function_name,
                                                          effect.effect_function_parameters)
        if function is None:
            output = ""
        ...
        if effect.value is not None:
            ...
            checking_functions = util.get_epistemic_world(self, effect.target_function_belief_sequence,
                                                          [history['functions'] for history in self.history])
            target_function = util.get_function_with_name_and_params(checking_functions, effect.target_function_name,
                                                                     effect.target_function_parameters)
